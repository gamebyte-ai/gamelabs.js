...
## TextStyle — text descriptor

Text content (used by `OscLabel`) is styled by a `TextStyle` (also exported from `@gamebyte/gamelabsjs` core, shared with future text-based widgets):

```ts
type TextStyle = {
  fontFamily?: string;
  fontSize?: number;
  fontWeight?: string; // CSS-style: "normal", "bold", "100"–"900"
  color?: number; // fill colour (hex, e.g. 0xffffff)
  alpha?: number; // 0..1
};
```

Stroke and shadow are intentionally not exposed yet — add when a real use case lands.

The framework registers an `osc.label` default with system-ui 16px normal-weight white text. Apps can `modify` it to retheme every label, or pass per-control overrides via the `text` field on `addControl`.
...
Labels are display-only controls — styled text with an optional sprite background. They're registered with the manager just like buttons and joysticks, support the same `setControlEnabled` / `setControlVisible` flags, and their text content is driven through `manager.setLabelText`.

### Configuration

```ts
type VirtualLabelConfig = VirtualControlConfig & {
  type: ControlType.Label;
  content: string; // initial text; updated via manager.setLabelText
  anchorX?: number; // 0..1 horizontal pivot, default 0 (left)
  anchorY?: number; // 0..1 vertical pivot, default 0 (top)
  text?: TextStyle; // text styling override
  bg?: SpriteStyle; // optional bg sprite — sized to text bounds × scaleX/Y
};
```

`anchorX` / `anchorY` set where the resolved screen anchor lands relative to the label's content — `(0, 0)` keeps the top-left at the anchor, `(0.5, 0.5)` centers on it, `(1, 1)` puts the bottom-right corner at the anchor.
...
// Update content at runtime — bg auto-resizes if present.
manager.setLabelText("wave", "WAVE 2");
```

### Centered announce text
...
manager.setControlVisible("announce", false); // hidden until needed

// On wave start:
manager.setLabelText("announce", `WAVE ${n}`);
manager.setControlVisible("announce", true);
```

### With a background sprite

Set `bg.textureId` to render a sprite under the text. The bg is sized to the rendered text bounds × `bg.scaleX/Y`, so values >= 1 give a padded badge:
...
manager.addControl({
  ...
  anchorX: 1,
  anchorY: 0,
  text: { color: 0x000000, fontSize: 14, fontWeight: "700" },
  bg: { textureId: MyAssetIds.BadgeBg, color: 0xffd700, alpha: 0.9, scaleX: 1.4, scaleY: 1.6 },
});
```

The bg auto-resizes when `setLabelText` is called.
...
| **Label**    | Whole label container dims to 50% alpha.                                             | n/a (display-only).                                      | n/a — no input state.                                                                |
...
│   ├── OnScreenControlsView.pixi.ts        ── orchestrator: resolves styles, instantiates widgets, positions, wires events
│   ├── OscButton.pixi.ts                   ── self-rendering button widget (extends StyledHudObject)
│   ├── OscJoystick.pixi.ts                 ── self-rendering joystick widget
│   └── OscLabel.pixi.ts                    ── self-rendering label widget (text + optional bg)
├── OnScreenControlsBinding.ts              ── module binding (DI + default asset/style requests)
├── OnScreenControlsAssetIds.ts             ── built-in texture ids
├── OnScreenControlTypes.ts                 ── enums, OSC config types (uses `SpriteStyle` + `TextStyle` from core)
└── index.ts
```
...
// Label content
manager.setLabelText(id, value);
manager.getLabelText(id);
...
- `OscButton`, `OscJoystick`, `OscLabel` — self-rendering widget classes (extend `StyledHudObject`)
- `OscButtonStyle`, `OscJoystickStyle`, `OscLabelStyle`, `OscStyleIds` — slot bundles registered in `StyleManager`
- `VirtualButtonConfig`, `VirtualJoystickConfig`, `VirtualLabelConfig`, `VirtualControlConfig`, `ControlConfig`
- `ControlType`, `ControlAnchor`
- `resolveAnchorPosition`

`SpriteStyle` and `TextStyle` are exported from the framework root (`@gamebyte/gamelabsjs`), not the OSC module — they're shared across all styled HUD widgets.
