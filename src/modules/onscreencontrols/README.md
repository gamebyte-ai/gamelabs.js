...
## OscVisual — the unified visual descriptor

Every textured slot on a control (button up / down / disabled / icon / progress, joystick base / knob) is configured by an `OscVisual`. Every field is optional; the view layer resolves omitted values from slot-aware defaults.
...
type OscVisual = {
  ...
  alpha?: number; // 0..1
  scale?: number; // fraction of slot size; 1 = fills the slot
};
```

Slot defaults (applied when the field is `undefined`):

| Slot                | textureId                                 | color      | alpha  | scale |
| ------------------- | ----------------------------------------- | ---------- | ------ | ----- |
| Button **up**       | `OnScreenControlsAssetIds.ButtonBg`       | `0x222222` | `0.5`  | `1`   |
| Button **down**     | `OnScreenControlsAssetIds.ButtonBg`       | `0x444444` | `0.8`  | `1`   |
| Button **disabled** | `OnScreenControlsAssetIds.ButtonBg`       | `0x4a5a4a` | `0.55` | `1`   |
| Button **icon**     | _required_                                | `0xFFFFFF` | `1`    | `0.6` |
| Button **progress** | `OnScreenControlsAssetIds.ButtonProgress` | `0xFFFFFF` | `0.85` | `1.1` |
| Joystick **base**   | `OnScreenControlsAssetIds.JoystickBase`   | `0xFFFFFF` | `0.85` | `1`   |
| Joystick **knob**   | `OnScreenControlsAssetIds.JoystickHandle` | `0xFFFFFF` | `0.95` | `1`   |

The progress ring's default `scale` is `1.1` so it sits 5% outside the bg circle. The button icon's default `scale` is `0.6` so it sits inside the bg with margin.
...
The textures are pure white so the runtime tint (`OscVisual.color`) controls the final colour. Apps override either by:
...
1. **Overriding the URL** (keeps the framework asset id):
   ...
   ```
2. **Using a different asset id per control** (per-slot textureId on `OscVisual`):
   ```ts
...
type VirtualButtonConfig = VirtualControlConfig & {
  ...
  up?: OscVisual; // resting visual
  down?: OscVisual; // pressed visual
  disabled?: OscVisual; // visual while `setControlEnabled(id, false)`
  icon?: OscVisual; // optional overlay (must supply textureId)
  progress?: OscVisual; // optional progress ring config
};
```
...
The ring is drawn behind the button bg so the visible ring forms a halo around the button. Override the `progress` `OscVisual` to retune colour, alpha, scale, or texture per button.
...
type VirtualJoystickConfig = VirtualControlConfig & {
  ...
  base?: OscVisual; // base ring visual
  knob?: OscVisual; // knob disk visual
};
```
...
| ------------ | ------------------------------------------------------------------------------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **Button**   | Bg uses the `disabled` `OscVisual`; cursor flips to default; icon alpha halved.      | `setButtonDown` is silently dropped.                     | If held, `setButtonUp` fires (key handler sees release).                             |
...
├── OnScreenControlTypes.ts                 ── enums, OscVisual, config types
└── index.ts
```
...
manager.addControl({
  ...
  icon: { textureId: MyAssetIds.SlowIcon, scale: 0.55 },
});
...
manager.addControl({
  ...
  up:       { textureId: MyAssetIds.ShopIdle,    color: 0xFFFFFF, alpha: 1, scale: 1.1 },
  down:     { textureId: MyAssetIds.ShopPressed, color: 0xFFFFFF, alpha: 1, scale: 1.05 },
  disabled: { textureId: MyAssetIds.ShopIdle,    color: 0x666666, alpha: 0.5, scale: 1.1 },
});
```
